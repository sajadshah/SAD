...
if __name__ == '__main__':
    if ('--help' in sys.argv) or ('-h' in sys.argv):
        ...
        opts, args = getopt.getopt(sys.argv[1:], "hm:e:l:c:b:t:o:r", ['bottleneck-featrues', 'file-based-train', 'additional-train-data', 'no-normal'])
        for opt, arg in opts:
            if opt == '-m':
                model = arg
            elif opt == '-e':
...
train_features_folder = os.path.join(datasetFolder, 'train','feature files')
trainFeatureFilesList = os.listdir(train_features_folder)

valid_features_folder = os.path.join(datasetFolder, 'validation', 'feature files')
validFeatureFilesList = os.listdir(valid_features_folder)
...
while epoch < num_epochs:
    ...
    for batch_files in iterate_files(trainFeatureFilesList, batchsize=52, shuffle=True):
        x_data = []
        y_data = []
        for featureFile in batch_files:
            train_file_index += 1
            if (btlnkFeatures):
                None
            else:
                x, y, n_dim_t = dataset.loadDataFile(
                    logger,
                    featureFile=os.path.join(train_features_folder, featureFile),
                    normalize=True,
    ...
    valid_batches = 0
    valid_acc = 0
    for valid_file_index in range(len(validFeatureFilesList)):
        featureFile = validFeatureFilesList[valid_file_index]
        ...
        if (btlnkFeatures):
            ...
            x, y, n_dim_t = dataset.loadDataFile(
                logger,
                featureFile=os.path.join(valid_features_folder, featureFile),
                normalize=True,
                sharedVariable=False,
                reshapeToChannels=True,
                n_context_frames=num_context_frames)
